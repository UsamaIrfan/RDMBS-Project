...
const Products = () => {
  ...
  useEffect(() => {
    const unsubs = async () => {
      setIsLoading(true)
      await getProducts((currentPage - 1) * NUMBER_OF_SALES_PER_SCREEN)
  ...
  useEffect(() => {
    setNumberOfPages(products?.length > 0 ? Math.ceil(products.length / NUMBER_OF_SALES_PER_SCREEN) : null)
  })



  const getProducts = async (skipCount, add) => {
    await dispatch(getAllSales(NUMBER_OF_SALES_PER_SCREEN, skipCount, add))
  }
  ...
  const getProductsTimeline = async () => {
    await axios.get(`${SERVER_API}/api/getSalesTimeline`,
      {
        ...
        <CCard>
              ...
              <CCol xl={3}>
                <CDropdown className="m-1">
                  <CDropdownToggle color="info">
                    Sales Per Page: {NUMBER_OF_SALES_PER_SCREEN}
                  </CDropdownToggle>
                  <CDropdownMenu onClick={(e) => setNUMBER_OF_SALES_PER_SCREEN(e.target.text)} style={{ maxHeight: "250px", overflowY: "scroll" }}>
              ...
              <CCol xl={3} className="p-3">
                <CDropdown className="m-1">
                  ...
                  <CDropdownMenu onClick={(e) => setProductsOrderBy(e.target.getAttribute("value"))} style={{ maxHeight: "250px", overflowY: "scroll" }}>
                    ...
                    <CDropdownItem value="product_id">Sale ID</CDropdownItem>
          ...
          <CCardBody>
            ...
            <CDataTable
              items={products}
              fields={[
                { key: 'Sale_Id', _classes: 'font-weight-bold' },
                'Sale_date', 'Sale_totalAmount', 'Sale_status'
              ]}
              hover
              striped
              itemsPerPage={NUMBER_OF_SALES_PER_SCREEN}
              activePage={page}
              clickableRows
              ...
              scopedSlots={{
                ...
                'Sale_status':
                  (item) => (
                    <td>
                      <CBadge color={getBadge('completed')}>
                        {'completed'}
                      </CBadge>
                    </td>
                  )
